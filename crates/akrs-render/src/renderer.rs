...
/// Load the bundled Chinese font, falling back to platform system fonts.
///
/// Returns `(primary_font, fallback_font)`.  The primary font is the first
/// candidate that passes verification (e.g. embedded NotoSansSC).  The
/// fallback font is the first *system* font that passes verification, so
...
fn load_font_with_fallback() -> (Option<Font>, Option<Font>) {
    ...
    // 1. Embedded font (compiled into the binary).
    ...
    match load_ttf_font_from_bytes(embedded) {
        Ok(f) => {
            if let Some(f) = verify_font(f) {
                eprintln!("[akrs-render] Chinese font loaded (embedded)");
                primary = Some(f);
            }
        }
        Err(e) => {
    ...
    // 2. Runtime-bundled font file.
    if primary.is_none() {
        let bundled_path = "assets/fonts/NotoSansSC-Regular-subset.ttf";
        if let Ok(bytes) = std::fs::read(bundled_path) {
            match load_ttf_font_from_bytes(&bytes) {
                Ok(f) => {
                    if let Some(f) = verify_font(f) {
                        eprintln!("[akrs-render] Chinese font loaded (runtime file)");
                        primary = Some(f);
                    }
                }
                Err(e) => {
                    eprintln!("[akrs-render] Runtime font file load failed: {:?}", e);
                }
            }
        }
    }

    // 3. System fonts — try every candidate and keep the first one that
    //    works as the fallback font.
...
/// Default window height.  On Windows the taskbar usually occupies ~40 px,
/// so we reserve a small margin to keep the bottom HUD buttons visible.
const WINDOW_HEIGHT: i32 = 1040;
...
pub async fn run(mut engine: Engine) {
    ...
    loop {
        ...
        if ui_mode == UiMode::AutoSavePrompt {
            ...
            draw_autosave_prompt(&engine, &mut buttons, sw, sh, &font);
        } else if ui_mode == UiMode::SettingsMenu {
            // Interactive settings: draw the scene dimmed behind the panel,
            // then render the settings controls and handle their interaction
            // directly. Sliders/toggles/dropdown are not button-based, so the
            // generic click handler is skipped for this mode (see below).
            draw_scene(&engine, &mut assets, sw, sh, true, &font).await;
            draw_dim_overlay(sw, sh, 0.7);
            let layout = compute_settings_layout(sw, sh);
            draw_settings_menu(&mut engine, &layout, &font, dropdown_open);
            handle_settings_interaction(
                &mut engine, &layout, &mut dragging_slider, &mut ui_mode, &mut dropdown_open,
            );
        } else if ui_mode != UiMode::Normal {
            // Draw game behind menu dimmed
            draw_scene(&engine, &mut assets, sw, sh, true, &font).await;
            draw_dim_overlay(sw, sh, 0.7);
            match ui_mode {
                UiMode::SaveMenu => draw_save_menu(&engine, &mut buttons, sw, sh, &font),
                UiMode::LoadMenu => draw_load_menu(&engine, &mut buttons, sw, sh, &font),
                _ => {}
            }
        } else if engine.phase() == EnginePhase::Title {
            draw_title_screen(&mut buttons, sw, sh, &mut assets, &font);
        } else if engine.phase() == EnginePhase::StoryEnded {
            draw_scene(&engine, &mut assets, sw, sh, true, &font).await;
            draw_story_ended(sw, sh, &mut buttons, &font);
        } else {
            ...
            draw_scene(&engine, &mut assets, sw, sh, !hud_hidden, &font).await;
            if !hud_hidden {
                draw_hud_buttons(&mut buttons, sh, &font);
            }
        }
...
fn draw_title_screen(buttons: &mut Vec<ButtonRect>, sw: f32, sh: f32, _assets: &mut AssetManager, font: &Option<Font>) {
    ...
    for (label, action) in &labels {
        draw_button(btn_x, btn_y, btn_w, btn_h, label, buttons, *action, font);
        btn_y += btn_h + 20.0;
    }
}

async fn draw_scene(engine: &Engine, assets: &mut AssetManager, sw: f32, sh: f32, show_ui: bool, font: &Option<Font>) {
    ...
    // Draw characters
    draw_characters(scene, assets, sw, sh, font).await;
    ...
    if let Some(dialogue) = &scene.dialogue {
        draw_dialogue(dialogue, sw, sh, font);
    }

    // Draw choices
    if let Some(choices) = &scene.choices {
        draw_choices(choices, sw, sh, font);
    }
}
...
async fn draw_characters(scene: &SceneState, assets: &mut AssetManager, sw: f32, sh: f32, font: &Option<Font>) {
    for char_state in &scene.characters {
        ...
        if let Some(tex) = assets.get_texture(AssetKind::Character, &sprite_name).await {
            let tex_w = tex.width();
            let tex_h = tex.height();
            let scale = (sh * 0.8) / tex_h;
            let draw_w = tex_w * scale;
            let draw_h = tex_h * scale;
            let x = sw * x_frac - draw_w / 2.0 + char_state.offset_x;
            let y = sh - draw_h - 50.0; // Stand on the bottom
            draw_texture_ex(
                tex.clone(),
                x,
                y,
...
fn draw_dialogue(dialogue: &akrs_runtime::DialogueState, sw: f32, sh: f32, font: &Option<Font>) {
    let box_h = 250.0;
    ...
    // Border
    draw_rectangle_lines(box_x, box_y, box_w, box_h, 2.0, Color::new(0.6, 0.6, 0.8, 0.8));
    ...
    draw_text_wrapped(
        &displayed,
        box_x + 20.0,
        box_y + (if dialogue.speaker.is_empty() { 30.0 } else { 70.0 }),
        box_w - 40.0,
        26.0,
        WHITE,
        font,
...
fn draw_choices(choices: &akrs_runtime::ChoicesState, sw: f32, sh: f32, font: &Option<Font>) {
    // Prompt
    if let Some(prompt) = &choices.prompt {
        let pw = measure_text_f(prompt, font, 32, 1.0).width;
        draw_text_f(
            prompt,
            (sw - pw) / 2.0,
            sh * 0.2,
    ...
    let total_h = choices.options.len() as f32 * (opt_h + 15.0);
    ...
    for (i, opt) in choices.options.iter().enumerate() {
        ...
        draw_rectangle_lines(opt_x, opt_y, opt_w, opt_h, 2.0,
        ...
        let tw = measure_text_f(&opt.text, font, 24, 1.0).width;
        draw_text_f(
            &opt.text,
            opt_x + (opt_w - tw) / 2.0,
            opt_y + 38.0,
...
fn draw_story_ended(sw: f32, sh: f32, buttons: &mut Vec<ButtonRect>, font: &Option<Font>) {
    draw_dim_overlay(sw, sh, 0.8);
    let text = "故事结束";
    let tw = measure_text_f(text, font, 64, 1.0).width;
    draw_text_f(text, (sw - tw) / 2.0, sh * 0.4, 64.0, WHITE, font);
...
fn draw_autosave_prompt(engine: &Engine, buttons: &mut Vec<ButtonRect>, sw: f32, sh: f32, font: &Option<Font>) {
    ...
    let dialog_w = 720.0_f32.min(sw - 80.0);
    let dialog_h = 360.0_f32.min(sh - 80.0);
    ...
    let center_x = sw / 2.0;
    let mut cursor_y = dialog_y + 56.0;

    // Title.
    let title = "检测到未正常退出";
    let tw = measure_text_f(title, font, 36, 1.0).width;
    draw_text_f(
        title,
        center_x - tw / 2.0,
        cursor_y,
    ...
    let line2 = "是否继续上次的游戏？";
    let l1w = measure_text_f(line1, font, 24, 1.0).width;
    let l2w = measure_text_f(line2, font, 24, 1.0).width;
    draw_text_f(line1, center_x - l1w / 2.0, cursor_y, 24.0, WHITE, font);
    cursor_y += 36.0;
    draw_text_f(line2, center_x - l2w / 2.0, cursor_y, 24.0, WHITE, font);
    cursor_y += 40.0;
    ...
    if let Some(save) = engine.saves().load_autosave().ok() {
        ...
        let sw2 = measure_text_f(&summary, font, 22, 1.0).width;
        draw_text_f(
            &summary,
            center_x - sw2 / 2.0,
            cursor_y,
    ...
    let btn_y = dialog_y + dialog_h - btn_h - 36.0;

    draw_button(btn1_x, btn_y, btn_w, btn_h, "继续游戏", buttons, ButtonAction::ContinueAutosave, font);
    draw_button(btn2_x, btn_y, btn_w, btn_h, "重新开始", buttons, ButtonAction::DiscardAutosave, font);
}

// ─── Menu drawing ───

fn draw_save_menu(engine: &Engine, buttons: &mut Vec<ButtonRect>, sw: f32, sh: f32, font: &Option<Font>) {
    draw_panel(sw, sh, "保存游戏", font);

    let saves = engine.saves();
    let max_slots = saves.max_slots().min(8);

    let slot_w = 600.0;
    let slot_h = 70.0;
    let slot_x = (sw - slot_w) / 2.0;
    let mut slot_y = sh * 0.2;

    for slot in 0..max_slots {
        let label = if saves.has_save(slot) {
            format!("存档位 {} — 点击覆盖", slot + 1)
        } else {
            format!("存档位 {} — 空", slot + 1)
        };
        draw_button(slot_x, slot_y, slot_w, slot_h, &label, buttons, ButtonAction::SaveSlot(slot), font);
        slot_y += slot_h + 10.0;
    }

    draw_button(slot_x, slot_y + 20.0, slot_w, slot_h, "返回", buttons, ButtonAction::CloseMenu, font);
}

fn draw_load_menu(engine: &Engine, buttons: &mut Vec<ButtonRect>, sw: f32, sh: f32, font: &Option<Font>) {
    draw_panel(sw, sh, "读取存档", font);

    let saves = engine.saves();
    let all_saves = saves.list_saves();
    let has_any = all_saves.iter().any(|s| s.is_some());

    if !has_any {
        let msg = "未找到存档文件。";
        let mw = measure_text_f(msg, font, 28, 1.0).width;
        draw_text_f(msg, (sw - mw) / 2.0, sh * 0.4, 28.0, Color::new(0.7, 0.7, 0.7, 1.0), font);
    } else {
        let slot_w = 600.0;
        let slot_h = 70.0;
        let slot_x = (sw - slot_w) / 2.0;
        ...
        for slot in 0..all_saves.len().min(8) {
            if let Some(meta) = &all_saves[slot] {
                let label = format!(
                    "存档位 {} — {} — {} — {}",
                    slot + 1,
                    meta.section_name,
                    format_play_time(meta.play_time_secs),
                    &meta.description.chars().take(40).collect::<String>(),
                );
                draw_button(slot_x, slot_y, slot_w, slot_h, &label, buttons, ButtonAction::LoadSlot(slot), font);
            } else {
                let label = format!("存档位 {} — 空", slot + 1);
                draw_button(slot_x, slot_y, slot_w, slot_h, &label, buttons, ButtonAction::LoadSlot(slot), font);
                // Disable visually
                draw_rectangle(slot_x, slot_y, slot_w, slot_h, Color::new(0.0, 0.0, 0.0, 0.5));
            }
            slot_y += slot_h + 10.0;
        }
    }
    ...
    let slot_w = 600.0;
    let slot_x = (sw - slot_w) / 2.0;
    draw_button(slot_x, sh * 0.85, slot_w, 50.0, "返回", buttons, ButtonAction::CloseMenu, font);
}
...
/// Compute the settings menu layout from the current screen size.
fn compute_settings_layout(sw: f32, sh: f32) -> SettingsLayout {
    let panel_x = sw * 0.2;
    let panel_y = sh * 0.15;
    let panel_w = sw * 0.6;
    let panel_h = sh * 0.7;
    let label_x = panel_x + 40.0;
    let control_x = panel_x + 300.0;
    let track_w = (panel_w - 300.0 - 240.0).max(120.0);
    let value_x = control_x + track_w + 20.0;
...
/// Draw the interactive settings menu. Reads live values from the engine so
/// dragging a slider is reflected immediately.
fn draw_settings_menu(engine: &mut Engine, layout: &SettingsLayout, font: &Option<Font>, dropdown_open: bool) {
    ...
    let title = "设置";
    let tw = measure_text_f(title, font, 48, 1.0).width;
    draw_text_f(
        title,
    ...
    draw_text_f("文本速度", layout.label_x, layout.row_mids[0] + 8.0, 24.0, WHITE, font);
    draw_slider_track(layout.slider_tracks[0], settings.text_speed / 999.0);
    draw_text_f(
        &format!("{:.0} 字/秒", settings.text_speed),
        layout.value_x,
        layout.row_mids[0] + 8.0,
        22.0,
        WHITE,
        font,
    );
    ...
    draw_text_f("BGM 音量", layout.label_x, layout.row_mids[1] + 8.0, 24.0, WHITE, font);
    draw_slider_track(layout.slider_tracks[1], settings.bgm_volume);
    draw_text_f(
        &format!("{:.0}%", settings.bgm_volume * 100.0),
        layout.value_x,
        layout.row_mids[1] + 8.0,
        22.0,
        WHITE,
        font,
    );
    ...
    draw_text_f("音效音量", layout.label_x, layout.row_mids[2] + 8.0, 24.0, WHITE, font);
    draw_slider_track(layout.slider_tracks[2], settings.sfx_volume);
    draw_text_f(
        &format!("{:.0}%", settings.sfx_volume * 100.0),
        layout.value_x,
        layout.row_mids[2] + 8.0,
        22.0,
        WHITE,
        font,
    );
    ...
    draw_text_f("自动恢复", layout.label_x, layout.row_mids[3] + 8.0, 24.0, WHITE, font);
    draw_toggle(layout.toggles[0], settings.auto_recovery, font);

    // Row 5: fullscreen toggle.
    draw_text_f("全屏模式", layout.label_x, layout.row_mids[4] + 8.0, 24.0, WHITE, font);
    draw_toggle(layout.toggles[1], settings.fullscreen, font);

    // Row 6: resolution dropdown.
    draw_text_f("分辨率", layout.label_x, layout.row_mids[5] + 8.0, 24.0, WHITE, font);
    draw_dropdown(layout.dropdown, settings.resolution, font, dropdown_open);
...
fn draw_slider_track(track: Rect4, fraction: f32) {
    ...
    draw_circle(knob_x, knob_y, 9.0, Color::new(0.9, 0.85, 1.0, 1.0));
}

/// Draw an on/off toggle switch.
fn draw_toggle(r: Rect4, on: bool, font: &Option<Font>) {
    ...
    draw_rectangle_lines(r.x, r.y, r.w, r.h, 1.5, fg);
    let tw = measure_text_f(label, font, 18, 1.0).width;
    draw_text_f(label, r.x + (r.w - tw) / 2.0, r.y + r.h / 2.0 + 6.0, 18.0, fg, font);
}

/// Draw the resolution dropdown. When `open` is true the full list of
/// presets is rendered below the current-value box.
fn draw_dropdown(r: Rect4, resolution: (u32, u32), font: &Option<Font>, open: bool) {
    // Current value box.
    draw_rectangle(r.x, r.y, r.w, r.h, Color::new(0.15, 0.12, 0.22, 0.9));
    draw_rectangle_lines(r.x, r.y, r.w, r.h, 1.5, Color::new(0.5, 0.45, 0.7, 0.8));
    let label = format!("{}x{}", resolution.0, resolution.1);
    draw_text_f(&label, r.x + 12.0, r.y + r.h / 2.0 + 7.0, 20.0, WHITE, font);
    // Dropdown arrow.
    ...
    if open {
        ...
        // List background.
        draw_rectangle(r.x, r.y + r.h, r.w, list_h, Color::new(0.12, 0.1, 0.2, 0.95));
        draw_rectangle_lines(r.x, r.y + r.h, r.w, list_h, 1.0, Color::new(0.5, 0.45, 0.7, 0.6));
        for (i, (w, h)) in presets.iter().enumerate() {
            ...
            draw_text_f(&item_label, r.x + 12.0, iy + item_h / 2.0 + 6.0, 18.0, color, font);
        }
    }
}
...
fn draw_panel(sw: f32, sh: f32, title: &str, font: &Option<Font>) {
    let panel_x = sw * 0.1;
    let panel_y = sh * 0.1;
    let panel_w = sw * 0.8;
    let panel_h = sh * 0.8;
    draw_rectangle(panel_x, panel_y, panel_w, panel_h, Color::new(0.05, 0.05, 0.1, 0.95));
    draw_rectangle_lines(panel_x, panel_y, panel_w, panel_h, 2.0, Color::new(0.5, 0.5, 0.7, 0.8));

    let tw = measure_text_f(title, font, 48, 1.0).width;
    draw_text_f(title, (sw - tw) / 2.0, panel_y + 60.0, 48.0, WHITE, font);
}
...
fn handle_click(
    ...
    if *ui_mode == UiMode::Normal {
        if engine.phase() == EnginePhase::Title {
            ...
            handle_choice_click(mx, my, engine);
        } else if engine.phase() == EnginePhase::StoryEnded {
            // Ignore
        } else {
...
fn handle_choice_click(mx: f32, my: f32, engine: &mut Engine) {
...
fn draw_hud_buttons(buttons: &mut Vec<ButtonRect>, sh: f32, font: &Option<Font>) {
    let btn_w = 100.0;
    let btn_h = 36.0;
    let gap = 8.0;
    let start_x = 20.0;
    ...
    for (label, action) in &hud_buttons {
        draw_small_button(x, start_y, btn_w, btn_h, label, buttons, *action, font);
        x += btn_w + gap;
    }
}
...
fn draw_small_button(
    ...
    draw_rectangle(x, y, w, h, bg_color);
    draw_rectangle_lines(x, y, w, h, 1.5, if hover {
...
fn draw_text_wrapped(text: &str, x: f32, y: f32, max_w: f32, font_size: f32, color: Color, font: &Option<Font>) {
    let mut current_y = y;
