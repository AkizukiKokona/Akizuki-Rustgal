...
[![Version](https://img.shields.io/badge/version-2.0.98-success.svg)](#)
...
- **蓝图模式（可视化节点编辑）** — 工具栏「蓝图模式」按钮切换到类 Unreal Engine 蓝图的流程图编辑界面（基于 egui 自绘）。剧本的每一行变成画布上的一个节点（17 种类型，各自带颜色标识），节点之间用连线表示执行顺序。节点正文显示友好文本（自动去掉脚本前缀符号），超长文本软换行显示（不截断）。支持左键拖动节点移动、左键拖动输出引脚到输入引脚创建连线、右键拖动空白平移画布、Ctrl+滚轮光标锚点缩放、右键空白弹出 14 种节点模板菜单、右键节点弹出操作菜单、双击节点填空式编辑、Delete 删除选中节点、非触摸模式框选多选。自动布局按章节分列、一列纵向不限、列内水平居中对齐（连线保持垂直直线）。章节间连线用贝塞尔曲线（自动绕避节点，保持首尾相连逻辑顺序），章节内连线用正交折线（绕避节点）。选择块拖出时自动生成 Choice + 2 个 Option 节点 + 连线，结尾 `?` 自动补全。蓝图模式下右侧预览面板显示积木卡片预览（可拖拽到画布添加节点）。结局/结局声明模块无输出引脚（流程终止）。
...
- **蓝图填空式编辑** — 双击蓝图节点不再显示原始脚本文本，而是按节点类型显示填空式编辑器：立绘上场选立绘+填 xy 大小、立绘下场选角色、背景选背景+选过渡效果、音乐选音乐文件、跳转/访问选章节（从当前剧本解析）、对话分角色和内容两个输入框、旁白内容输入框。用户完全不接触代码格式，格式自动生成。编辑面板在节点附近就地显示，宽度自适应，遮挡时自动避让。立绘下场的角色名选择会从已连接的上游节点（立绘上场）提取，无连接时回退全画布扫描；若连接到不可用位置则警告并支持断开连接。
- **蓝图框选多选** — 非触摸模式下，鼠标在画布空白处左键拖拽画框，框内节点全部选中（黄色高亮），支持批量删除（Delete）和整体移动。
- **编辑器统一菜单栏** — 顶部工具栏从十几个按钮平铺改为统一结构：蓝图模式、对照翻译独立置顶，其余功能（项目/文件/编译/首页帮助关于）收纳进「菜单」下拉按钮，节省工具栏空间。
- **编辑器设置** — 新增编辑器设置窗口（菜单 → 编辑器设置），包含三个开关：进入编辑器默认打开上一次文件（默认开）、退出时自动保存文件不询问（默认关）、进入编辑器自动启动蓝图模式（默认开）。设置持久化到编辑器数据目录的 `editor_settings.json`，不随项目分发。
- **资源文件系统** — 左栏从单一脚本文件列表改为分类标签页（脚本 / 立绘 / 背景 / 音乐），打开项目后自动读取整个项目资源文件。双击图片资源在弹窗中预览，双击音乐资源在系统文件管理中打开。各资源标签页底部提供上传按钮。
- **关于页可配置** — 通过 `project.json` 的 `about` 字段（编辑器「项目设置 → 关于页配置」内可编辑）自定义游戏内关于页的图片、文本和颜色。图片支持自动同步 logo（用 `kokona.png`）或自定义路径（约定放在 `assets/about/` 目录）；文本行数可自由增减、逐行编辑；颜色分粗体行/正常行/强调行三组。各字段留空或缺失时回退默认值，旧项目文件无需改动。
