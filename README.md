...
- **存档系统** — 多槽位存档/读档，支持崩溃恢复；快速存档使用独立不可见槽位，不覆盖手动存档；读档/崩溃恢复时完整还原背景、立绘、音乐等场景状态（含过渡中途存档：存档时把待应用的过渡变更一并写入快照，读档后清空残留过渡状态，避免立绘"下不去"）；存档页每个槽位显示场景缩略图（按场景快照重绘）与玩家自定义备注
- **热重载** — 修改剧本后自动重新编译，无需重启游戏
- **快进模式** — 支持仅文本快进和包含语音快进两种模式
- **设置系统** — 文字速度、音量、分辨率、全屏等可配置项
- **多语言翻译系统** — 原文剧本不修改，翻译文件与原文一一对应；支持章节标题、对话、旁白、分支选项、角色名翻译；编辑器内置对照翻译模式（原文-译文并排，原文只读）；CLI 一键生成翻译骨架；运行时设置页实时切换语言；UI 文本翻译内置英文兜底，语言文件缺失或部分 key 未译时回退英文而非显示 raw key
