...
[![Version](https://img.shields.io/badge/version-1.0.81-success.svg)](#)
...
- **已读 / 未读文字着色** — 引擎记录每句对话/旁白是否被玩家看过（`saves/read_history.json`，以原文 `speaker|text` 为 key，跨语言稳定）。首次出现的句子视为「未读」，默认白色；再次出现视为「已读」，默认浅紫色。角色名与对白正文统一按已读/未读着色。同时修复了「仅跳过已读」快进此前用翻译后文本拼 key 导致永远判定为未读、快进卡死的缺陷——现直接复用对话的 `is_read` 标志。已读历史在引擎构造时即从磁盘加载、返回标题/故事结束/进入尾声重建引擎前先落盘，确保退出到标题再读档后已读标识不丢失。开发者模式标签页提供「将全部文本设为未读」一键清空已读历史。
...
- **章节切换动画** — `# 章节名 显示标题` 语法将章节名与显示标题分开（标题可含空格）。通过 `->` 跳转到带标题的章节时，游戏先播放 0.5 秒全屏淡入淡出（覆盖文本框等全部内容），随后从屏幕顶部滑入一条圆角矩形通知（半透明白底），章节名与标题分两行居中显示——第一行为章节标识符（常规字重），第二行为显示标题（加粗），停留 1 秒后按原路径滑出。文本过长时通知自动缩小字号以适配屏宽。编辑器在标题超过 24 字符时给出非阻断警告。`=>` 访问子章节不触发该通知。
...
- **Ctrl+点击跳转立绘预览** — 按住 Ctrl 键点击剧本中的 `+ 角色 ... at x,y size N` 立绘指令行时，右侧立绘预览面板立即跳转到该参数（角色名、pose、位置、大小均与代码一致），方便快速核对立绘效果。支持全部 `+` 指令语法：带/不带 pose、带/不带 `at`/`size`、中文位置词（居左/居中/居右）等。
- **立绘预览裁剪与位置统一** — 立绘预览面板增加裁剪矩形，超出预览区域的立绘部分不再溢出绘制；Y 位置拖动统一使用中心点对齐公式，消除了拖到 1.0 时从底部跳回顶部的突变。
- **Ctrl+点击扩展支持命令行** — Ctrl+点击 `@bg 背景` / `@music 音乐` 等命令行时，右侧预览面板自动跳转到对应标签页并选中该资源；修复了 egui 的 TextEdit 将 Ctrl+点击解释为「选词」并消费点击事件导致无反应的问题（改用原始指针输入检测 + galley 坐标转换）。
- **蓝图模式（可视化节点编辑）** — 工具栏新增「蓝图模式」按钮，点击进入类 Unreal Engine 蓝图的流程图编辑界面（精简版）。剧本的每一行变成一个可视节点（章节 / 对话 / 旁白 / 命令 / 立绘 / 选择 / 跳转 / 访问 / 返回 / 等待 / 结局等 15 种类型，各自带颜色标识），节点之间用贝塞尔曲线连线表示执行顺序。交互参考 UE 蓝图：左键拖动节点 = 移动、左键拖动输出引脚到输入引脚 = 创建连线、右键拖动空白 = 平移画布、右键点击空白 = 弹出操作菜单（按结构 / 内容 / 演出 / 流程分类的 13 种节点模板 + 整理布局 + 从脚本导入 + 删除选中节点）、Delete = 删除选中节点、Escape = 关闭菜单。节点宽度根据内容自适应，超长行用省略号截断。**首次自动整理布局**：每个剧本文件第一次用蓝图模式打开时自动整理布局（按章节分组、每列最多 8 个节点、节点高度自适应不重叠），整理记录持久化到编辑器数据目录（`blueprint_layout_done.txt`），之后再次进入不会打乱用户手动调整过的布局。提供「从脚本导入」「生成脚本」「整理布局」「清空」四个操作；进入蓝图时自动从当前脚本生成节点，退出时自动导出为脚本文本。**蓝图模式与预览面板联动**：右侧立绘 / 背景 / 音乐预览面板在蓝图模式下，原本的「追加到脚本」按钮自动变为「添加为节点」，把生成的语法直接作为节点加到画布上（而非插入脚本文本）；「替换插入」按钮变为「替换选中节点」，把选中节点的文本替换为新生成的语法。工具栏的「插入语法」下拉菜单在蓝图模式下也变为「插入节点」，点击直接添加节点。
...
点击工具栏「蓝图模式」按钮进入可视化节点编辑界面（灵感来自 Unreal Engine 蓝图，精简版）。剧本的每一行被转换为画布上的一个节点，节点之间用贝塞尔曲线连线表示执行顺序，比逐行点击插入更直观。

**节点类型**（15 种，各自带颜色标识）：章节（`#`）、对话（`角色:`）、旁白（`"..."`）、命令（`@bg`/`@music` 等）、立绘（`+`/`-`）、选择（`?`）、跳转（`->`）、访问（`=>`）、返回（`<=`）、等待（`~~`）、结局（`end`）、结局声明（`ending`）、解锁（`unlock`）、变量（`$`）、其他。
...
| 左键点击节点 | 选中（底部出现文本编辑栏） |
| 左键拖动节点 | 移动节点位置 |
| 左键拖动输出引脚（底部）→ 输入引脚（顶部） | 创建连线 |
| 右键拖动空白 | 平移画布 |
| 右键点击空白（不拖动） | 弹出添加节点菜单（13 种模板） |
| Delete | 删除选中节点 |
| Escape | 关闭右键菜单 |

**工具栏操作**：从脚本导入（解析当前脚本文本生成节点 + 自动布局）、生成脚本（DFS 遍历连线图输出脚本文本）、整理布局（按章节分组自动排列）、清空。
...
进入蓝图模式时自动从当前脚本导入节点，退出时自动导出为脚本文本。右侧预览面板在蓝图模式下仍然可见，方便随时预览效果。
